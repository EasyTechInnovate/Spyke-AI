...
export default function CounterOfferModal({ 
  ...
  useEffect(() => {
    if (isOpen) {
      trackEvent(ANALYTICS_EVENTS.COMMISSION_MODAL_OPENED, {
        current_offer: currentOffer,
        seller_id: sellerData?.id,
        product_id: productData?.id
      });
    }
  }, [isOpen, currentOffer, sellerData?.id, productData?.id]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateForm()) {
      trackEvent(ANALYTICS_EVENTS.COMMISSION_VALIDATION_FAILED, {
    ...
    try {
      trackEvent(ANALYTICS_EVENTS.COMMISSION_COUNTER_SUBMITTED, {
        original_offer: currentOffer,
        counter_offer: parseFloat(counterOffer),
        difference: parseFloat(counterOffer) - currentOffer,
        reasoning_length: reasoning.length,
        seller_id: sellerData?.id,
        product_id: productData?.id
      });
      ...
      trackEvent(ANALYTICS_EVENTS.COMMISSION_SUBMISSION_FAILED, {
        error: error.message,
        counter_offer: counterOffer
      });
  ...
  const handleClose = () => {
    trackEvent(ANALYTICS_EVENTS.COMMISSION_MODAL_CLOSED, {
      had_input: counterOffer.length > 0 || reasoning.length > 0,
      time_spent: Date.now() - modalOpenTime
    });
    onClose();
  };
            ...
            {counterOffer && productData?.price && (
              ...
              >
                ...
                <p className="text-2xl font-bold text-green-600">
                  ${calculatePotentialEarnings().toFixed(2)}
                </p>
                <p className="text-xs text-green-600 mt-1">
                  Per sale at {counterOffer}% commission
                </p>
              </motion.div>
            )}
