...
function ProductPerformanceChart({ products }) {
    ...
    const [metric, setMetric] = useState('views') // views | sales | revenue | conversion
    const [showAll, setShowAll] = useState(false)
    // Decide how many to show when not expanded
    ...
    // Sort by currently selected metric desc
    ...
    const rows = showAll ? sorted : sorted.slice(0, COLLAPSED_COUNT)
    if (!hasData)
        return (
            <Section
    ...
    // Determine scaling based on selected metric
    const maxMetric = Math.max(...rows.map((r) => r[metric]), 1)
    const totalViews = prepared.reduce((s, r) => s + r.views, 0)
    const totalSales = prepared.reduce((s, r) => s + r.sales, 0)
    const totalRevenue = prepared.reduce((s, r) => s + r.revenue, 0)
    const avgConversion = prepared.length ? prepared.reduce((s, r) => s + r.conversion, 0) / prepared.length : 0
    return (
        <Section
            title="Product Performance"
            icon={<BarChart3 className="h-5 w-5 text-[#00FF89]" />}
            right={
                <div className="flex items-center gap-3 text-xs text-gray-400">
                    ...
                    {prepared.length > COLLAPSED_COUNT && (
                        <button
                            onClick={() => setShowAll((v) => !v)}
                            className="rounded-lg border border-gray-700 bg-gray-800 px-2 py-1 text-xs text-gray-200 hover:border-[#00FF89]">
                            {showAll ? 'Show Top' : `Show All (${prepared.length})`}
                        </button>
                    )}
                </div>
            }>
            <div className="relative">
                <div className="absolute inset-0 top-6">
                    {[0, 25, 50, 75, 100].map((p) => (
                        <div
                            key={p}
                            className="absolute inset-x-0 border-t border-gray-700/40"
                            style={{ top: `${p}%` }}
                        />
                    ))}
                </div>
                {/* Horizontal scroll container so large product sets are manageable */}
                <div className="relative mt-2 flex max-h-[300px] overflow-x-auto pb-4">
                    <div className="flex items-end gap-4 pl-1 pr-4">
                        {rows.map((r, i) => {
                                    ...
                                    <div className="relative flex h-48 w-full items-end">
                                        <div className="relative w-10 flex-1 rounded-t-md bg-gray-800">
                                            <motion.div
                                                ...
                                                className="absolute bottom-0 w-full rounded-t-md bg-gradient-to-t from-[#00FF89] to-[#00E67A]"
                                            />
                                            <div className="pointer-events-none absolute -top-6 left-1/2 -translate-x-1/2 rounded bg-gray-900 px-2 py-0.5 text-[10px] text-white opacity-0 shadow-sm transition-opacity group-hover:opacity-100">
                                                {metric === 'conversion'
                                                    ? value.toFixed(1) + '%'
                                                    : metric === 'revenue'
                                        ...
                                        {metric !== 'views' && (
                                            <div className="ml-1 flex w-2 flex-col justify-end rounded bg-gray-800">
                                                <motion.div
                                                    ...
                                                    className="w-full rounded bg-[#00FF89]"
                                                    title={`${r.views.toLocaleString()} views`}
                                                />
                                            </div>
                                        )}
                                        {metric !== 'sales' && (
                                            <div className="ml-1 flex w-2 flex-col justify-end rounded bg-gray-800">
                                                <motion.div
                                                    ...
                                                    title={`${r.sales.toLocaleString()} sales`}
                                                />
                                            </div>
                                        )}
                                    </div>
            ...
            {/* Summary KPIs */}
            <div className="mt-4 grid grid-cols-2 gap-3 border-t border-gray-700 pt-4 text-center md:grid-cols-4">
                <div>
                    <div className="text-lg font-bold text-[#00FF89]">{totalViews.toLocaleString()}</div>
                ...
                <div>
                    <div className="text-lg font-bold text-purple-400">${totalRevenue.toLocaleString()}</div>
                    <div className="text-[11px] text-gray-400">Total Revenue</div>
                </div>
                <div>
                    <div className="text-lg font-bold text-orange-400">{avgConversion.toFixed(1)}%</div>
                    <div className="text-[11px] text-gray-400">Avg Conversion</div>
                </div>
            </div>
            {prepared.length > COLLAPSED_COUNT && !showAll && (
                <div className="mt-3 text-center text-[11px] text-gray-500">
                    Showing top {COLLAPSED_COUNT} of {prepared.length} products by {metric}.
                </div>
            )}
        </Section>
    )
    ...
    if (!cats.length)
        return (
            <Section
                ...
                icon={<PieChart className="h-5 w-5 text-[#00FF89]" />}>
                <div className="grid place-items-center py-14 text-center text-gray-400">
...
function ProductStatusOverview({ products }) {
    ...
    return (
        <Section
            ...
            icon={<Activity className="h-5 w-5 text-[#00FF89]" />}>
...
function PriceAnalysisChart({ products }) {
    ...
    return (
        <Section
            ...
            icon={<DollarSign className="h-5 w-5 text-[#00FF89]" />}>
            <div className="relative h-64">
                <div className="absolute inset-0">
                    {[0, 25, 50, 75, 100].map((p) => (
                        <div
                            key={p}
                            className="absolute inset-x-0 border-t border-gray-700/50"
                            style={{ top: `${p}%` }}
                        />
                    ))}
                </div>
...
function DetailedProductCard({ product, index, categoryNames }) {
    ...
    return (
        <motion.div
            ...
            className="rounded-2xl border border-gray-700 bg-gray-900/60 p-5 hover:border-gray-600">
            ...
            <div className="mb-5 grid grid-cols-2 gap-3 md:grid-cols-4">
                <Stat
                    ...
                    color="text-blue-400"
                    label="Views"
                    value={Number(product.views ?? 0).toLocaleString()}
                />
                <Stat
                    icon={<ShoppingCart className="h-4 w-4" />}
                    color="text-green-400"
                    label="Sales"
                    value={Number(product.sales ?? 0).toLocaleString()}
                />
                <Stat
                    icon={<DollarSign className="h-4 w-4" />}
                    color="text-purple-400"
                    label="Revenue"
                    value={`$${Number(product.revenue ?? Number(product.price ?? 0) * Number(product.sales ?? 0)).toLocaleString()}`}
                />
                <Stat
                    icon={<TrendingUp className="h-4 w-4" />}
                    color="text-orange-400"
                    label="Conversion"
                    value={`${Number(product.conversionRate ?? 0).toFixed(2)}%`}
                />
            </div>
            <div className="mb-5 grid grid-cols-3 gap-3">
                <MiniStat
                    ...
                    label={`Rating (${Number(product.totalReviews ?? 0)})`}
                    value={Number(product.averageRating ?? 0).toFixed(1)}
                />
                <MiniStat
                    icon={<TrendingUp className="h-4 w-4" />}
                    color="text-blue-400"
                    label="Upvotes"
                    value={Number(product.upvotes ?? 0)}
                />
                <MiniStat
                    icon={<Package className="h-4 w-4" />}
                    color="text-red-400"
                    label="Favorites"
                    value={Number(product.favorites ?? 0)}
                />
            </div>
            <div className="mb-5 space-y-2">
                <Flag
                    label="Verified Product"
                    on={Boolean(product.isVerified)}
                />
                <Flag
                    label="Tested"
                    on={Boolean(product.isTested)}
                />
                <Flag
                    label="Refund Policy"
                    on={Boolean(product.hasRefundPolicy)}
                    onText="Available"
            ...
            {Array.isArray(product.tags) && product.tags.length > 0 && (
                <div className="mb-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                        ...
                        {product.tags.length > 6 && <span className="text-xs text-gray-400">+{product.tags.length - 6} more</span>}
                    </div>
                </div>
            )}
    ...
    return (
        <div className="space-y-8">
            <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
                <KPI
                    icon={<Package className="h-5 w-5 text-[#00FF89]" />}
                ...
                <KPI
                    ...
                    label="Total Views"
                    value={(summary.totalViews ?? products.reduce((s, p) => s + Number(p.views ?? 0), 0)).toLocaleString()}
                ...
                <KPI
                    ...
                    label="Avg Rating"
                    value={(
                        summary.avgRating ?? (products.length ? products.reduce((s, p) => s + Number(p.averageRating ?? 0), 0) / products.length : 0)
                    ).toFixed(1)}
                    sub="Overall rating"
                />
                <KPI
                    ...
                    label="Total Upvotes"
                    value={summary.totalUpvotes ?? products.reduce((s, p) => s + Number(p.upvotes ?? 0), 0)}
            ...
            <div className="sticky top-0 z-10 -mx-1 rounded-xl border border-gray-700 bg-gray-900/80 px-4 py-3 backdrop-blur supports-[backdrop-filter]:backdrop-blur">
                ...
                <div className="mt-2 flex flex-wrap gap-2">
                    ...
                    {cat && (
                        <Chip
                            label={`Category: ${categoryNames[cat] || cat}`}
                            onRemove={() => setCat('')}
                        />
                    )}
                    {status && (
            ...
            {view === 'cards' ? (
                <Section
                    ...
                    icon={<Rows className="h-5 w-5 text-[#00FF89]" />}
                    right={
