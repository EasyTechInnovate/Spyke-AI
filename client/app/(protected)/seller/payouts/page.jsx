        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
            ...
            <div className="flex flex-wrap gap-3 text-sm">
              ...
              <Badge label={`Eligible: ${earnings.isEligible ? 'Yes' : 'No'}`} color={earnings.isEligible ? 'emerald' : 'rose'} />
              {earnings.isOnHold && earnings.holdPeriodEnd && (
                <Badge label={`On Hold Until: ${new Date(earnings.holdPeriodEnd).toLocaleDateString()}`} color="amber" />
              )}
            </div>
            {!earnings.isEligible && (
          ...
          <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <FeeLine label="Platform Fee %" value={`${earnings.platformFeePercentage}%`} />
              <FeeLine label="Platform Fee" value={`$${Math.round(earnings.platformFee)}`} />
              <FeeLine label="Processing Fee" value={`$${Math.round(earnings.processingFee)}`} />
              <FeeLine label="Commission Rate" value={`${earnings.commissionRate}%`} />
            </div>
          </div>
        </div>
      )}
      {pending && (
        <div className="bg-white/5 border border-white/10 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <span className={`px-2 py-1 rounded-full border text-xs font-medium ${statusColorMap[pending.status] || 'bg-white/10 text-white/60 border-white/20'}`}>{pending.status}</span>
          </div>
          <div className="text-sm text-white/70">
            <div className="flex justify-between py-1"><span>Amount</span><span>${Math.round(pending.amount)}</span></div>
            <div className="flex justify-between py-1"><span>Requested</span><span>{new Date(pending.requestedAt).toLocaleDateString()}</span></div>
            {pending.approvedAt && <div className="flex justify-between py-1"><span>Approved</span><span>{new Date(pending.approvedAt).toLocaleDateString()}</span></div>}
          </div>
        ...
        <div className="overflow-x-auto -mx-3 px-3">
          <table className="w-full text-sm">
            ...
            <tbody>
              {loadingDashboard && (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-white/40"><Loader2 className="w-4 h-4 animate-spin inline" /></td>
                </tr>
              )}
              {!loadingDashboard && recent.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-white/40">No payout history yet</td>
                </tr>
              )}
              {recent.map(p => (
                <tr key={p._id} className="border-b border-white/5 last:border-none">
                  ...
                  <td className="py-2">
                    <span className={`px-2 py-1 rounded-full border text-xs font-medium ${statusColorMap[p.status] || 'bg-white/10 text-white/60 border-white/20'}`}>{p.status}</span>
                  </td>
